...
import { FiLogOut, FiUser, FiBell, FiBookOpen, FiDollarSign, FiCalendar, FiHeart, FiPhone, FiShield, FiTrendingUp } from 'react-icons/fi';
import { G, KPI, TabButton, PageNav, statusIcon, severityColor, notifTypeIcon } from '../parent/components';
...
export default function StudentDashboard() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [n, fp, fs, at, ex, hr, al, dc] = await Promise.all([
      ...
      setAllergies(al.data||[]); setDiscipline(dc.data||[]);
  ...
  const avgMarks = results.length > 0 ? Math.round(results.reduce((a: number, r: any) => a + Number(r.score || 0), 0) / results.length) : 0;
    ...
    <div className="min-h-screen bg-gray-50/80">
      ...
      <div className="bg-white border-b border-gray-100 shadow-sm sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl flex items-center justify-center text-xl shadow-lg" style={{ background: G.teal }}>🎓</div>
            <div><p className="font-extrabold text-gray-900 text-sm">AlphaSchool Portal</p><p className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">Student Dashboard</p></div>
          </div>
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-xl bg-teal-50 border border-teal-100"><FiUser size={14} className="text-teal-600" /><span className="text-xs font-bold text-teal-800">{session.full_name}</span></div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-5">
        ...
        <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm flex items-center gap-4 flex-wrap">
          <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-xl font-bold text-white shadow-lg" style={{ background: G.teal }}>{(st?.first_name?.[0]||'')+(st?.last_name?.[0]||'')}</div>
          <div className="flex-1 min-w-[200px]"><h2 className="text-lg font-extrabold text-gray-900">{st?.first_name} {st?.last_name}</h2><p className="text-xs text-gray-500">Adm: {st?.admission_number||'-'} · Form: {st?.school_forms?.form_name||st?.form_id||'-'}</p></div>
          <div className="flex gap-6">
            <div className="text-center"><p className="text-[10px] text-gray-400 uppercase font-bold">Attendance</p><p className={`text-xl font-extrabold ${attendRate>=80?'text-green-600':attendRate>=50?'text-amber-600':'text-red-600'}`}>{attendRate}%</p></div>
            <div className="text-center"><p className="text-[10px] text-gray-400 uppercase font-bold">Avg Marks</p><p className={`text-xl font-extrabold ${avgMarks>=50?'text-green-600':'text-red-600'}`}>{avgMarks}%</p></div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3">
          ...
          <KPI label="Avg Marks" value={`${avgMarks}%`} emoji="📊" color="#2563eb"/>
        ...
        <div className="flex gap-1 p-1 bg-gray-100 rounded-xl w-fit overflow-x-auto">
          {['overview|🏠 Overview','results|📊 Results','attendance|📅 Attendance','fees|💰 Fees','discipline|📋 Discipline','health|🏥 Health','notifications|🔔 Alerts'].map(t=>{const[k,l]=t.split('|');return <TabButton key={k} active={tab===k} onClick={()=>setTab(k)} label={l}/>;})}
        </div>

        {/* OVERVIEW */}
        {tab==='overview' && <div className="grid lg:grid-cols-3 gap-5">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden"><div className="px-5 py-4 border-b border-gray-100" style={{background:G.teal}}><h3 className="text-sm font-bold text-white">📊 Latest Results</h3></div><div className="divide-y divide-gray-50">{results.slice(0,6).map(r=>{const mk=Number(r.score||0);return <div key={r.id} className="px-5 py-3 flex items-center justify-between"><div><p className="text-xs font-bold text-gray-900">{r.school_subjects?.subject_name||'-'}</p><p className="text-[10px] text-gray-400">{r.exam_type||'-'}</p></div><span className={`text-xs font-extrabold ${mk>=50?'text-green-600':'text-red-600'}`}>{r.score||'-'}%</span></div>})}{results.length===0&&<div className="text-center py-8 text-gray-400 text-sm">No results</div>}</div></div>
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden"><div className="px-5 py-4 border-b border-gray-100" style={{background:G.green}}><h3 className="text-sm font-bold text-white">📅 Recent Attendance</h3></div><div className="divide-y divide-gray-50">{attendance.slice(0,6).map(a=><div key={a.id} className="px-5 py-3 flex items-center justify-between"><div><p className="text-xs font-bold text-gray-900">{a.attendance_date||'-'}</p></div><div className="flex items-center gap-2">{statusIcon(a.status)}<span className={`text-xs font-bold ${a.status==='Present'?'text-green-700':a.status==='Absent'?'text-red-700':'text-amber-700'}`}>{a.status||'-'}</span></div></div>)}{attendance.length===0&&<div className="text-center py-8 text-gray-400 text-sm">No records</div>}</div></div>
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden"><div className="px-5 py-4 border-b border-gray-100" style={{background:G.purple}}><h3 className="text-sm font-bold text-white">🔔 Alerts</h3></div><div className="divide-y divide-gray-50">{notifications.slice(0,6).map(n=><div key={n.id} onClick={()=>markRead(n.id)} className={`px-5 py-3 cursor-pointer hover:bg-gray-50 ${!n.is_read?'bg-blue-50/30':''}`}><p className={`text-xs font-bold ${!n.is_read?'text-gray-900':'text-gray-500'}`}>{notifTypeIcon(n.type)} {n.title}</p><p className="text-[10px] text-gray-400">{new Date(n.created_at).toLocaleString()}</p></div>)}{notifications.length===0&&<div className="text-center py-8 text-gray-400 text-sm">No alerts</div>}</div></div>
        </div>}

        {/* RESULTS */}
        {tab==='results' && <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100" style={{background:G.teal}}><h3 className="text-sm font-bold text-white">📊 My Exam Results</h3></div>
        ...
        {tab==='attendance' && <div className="space-y-5">
          <div className="grid sm:grid-cols-3 gap-3">
            ...
            <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm text-center" style={{borderLeftWidth:4,borderLeftColor:'#0d9488'}}><p className="text-[10px] font-bold text-gray-400 uppercase">Rate</p><p className="text-2xl font-extrabold text-teal-600 mt-1">{attendRate}%</p></div>
          </div>
        ...
        {/* DISCIPLINE */}
        {tab==='discipline' && <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          {discipline.length===0&&<div className="text-center py-12 text-gray-400"><p className="text-3xl mb-2">📋</p><p>No discipline records — Keep it up!</p></div>}
        </div>}
        ...
        {tab==='health' && <div className="space-y-5">
          {healthRec && <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100" style={{background:G.red}}><h3 className="text-sm font-bold text-white flex items-center gap-2"><FiHeart size={14}/> My Health Record</h3></div>
            <div className="p-5 grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="sm:col-span-2"><p className="text-[10px] font-bold text-gray-400 uppercase">Chronic Conditions</p><p className="text-sm text-gray-700 mt-1">{healthRec.chronic_conditions||'None'}</p></div>
