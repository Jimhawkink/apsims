...
export function ClassViewTab() {
  ...
  const { forms, streams, entries, subjects, termEntries, bTerm, bYear, getFormName, getStreamName, printTimetable, buildPrintRows, allPeriodsSorted } = ctx;
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
        ...
        <div className="flex flex-wrap gap-2">
          {subjects.filter(s => classEntries.some(e => e.subject_id === s.id)).map(s => {
            ...
            const cnt = classEntries.filter(e => e.subject_id === s.id).length;
            return <div key={s.id} className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold" style={{ background: color.bg, color: color.text, border: `1.5px solid ${color.border}` }}><div className="w-2 h-2 rounded-full" style={{ background: color.text }} />{s.subject_name} <span className="opacity-60">({cnt})</span></div>;
          })}
        </div>
      </div>
    </div>
...
export function TeacherViewTab() {
  const ctx = useTimetable();
  const { teachers, termEntries, entries, bTerm, bYear, getTeacherName, printTimetable, allPeriodsSorted, subjects, getSubjectName, getFormName, getStreamName } = ctx;
  const [tTeacher, setTTeacher] = useState('');

  const teacherEntries = tTeacher ? termEntries.filter(e => e.teacher_id === Number(tTeacher)) : [];
...
export function RoomViewTab() {
  const ctx = useTimetable();
  const { classrooms, termEntries, bTerm, bYear } = ctx;
...
export function MasterViewTab() {
  const ctx = useTimetable();
  const { forms, streams, termEntries, allPeriodsSorted, subjects, classKeys, bTerm, bYear, getSubjectCode, getTeacherShort, getFormName, getStreamName } = ctx;
