...
export default function PortalsPage() {
  ...
  const [uForm,setUForm]=useState({user_type:'parent',linked_student_id:0,username:'',full_name:'',email:'',phone:'',is_active:true,password:'',auto_password:true});
  ...
  const saveUser=async()=>{if(!uForm.username.trim()||!uForm.linked_student_id)return toast.error('🚫 Username & student are required!',{icon:'⚠️'});setSaving(true);try{const pw=uForm.auto_password?generatePassword():uForm.password.trim();if(!pw){setSaving(false);return toast.error('🔑 Password is required!',{icon:'⚠️'})}const p={user_type:uForm.user_type,linked_student_id:uForm.linked_student_id,username:uForm.username.trim(),full_name:uForm.full_name.trim()||null,email:uForm.email.trim()||null,phone:uForm.phone.trim()||null,is_active:uForm.is_active,password_hash:pw};let err;if(editItem?.id){const up:any={user_type:p.user_type,linked_student_id:p.linked_student_id,username:p.username,full_name:p.full_name,email:p.email,phone:p.phone,is_active:p.is_active};if(uForm.password.trim())up.password_hash=pw;({error:err}=await supabase.from('school_portal_users').update(up).eq('id',editItem.id))}else({error:err}=await supabase.from('school_portal_users').insert([p]));if(err)throw err;if(!editItem?.id){setGeneratedPw(pw);toast.success('🎉 Portal user created!',{duration:5000,icon:'✅'});toast.success(`🔑 Login: ${uForm.username} / ${pw}`,{duration:15000,icon:'📋'})}else toast.success('✨ Portal user updated!',{icon:'✅'});setShowModal('');fetchAll()}catch(e:any){toast.error(`💥 ${e.message}`,{icon:'❌'})}setSaving(false)};
  ...
  const toggleUser=async(u:any)=>{const{error}=await supabase.from('school_portal_users').update({is_active:!u.is_active}).eq('id',u.id);if(!error){toast.success(u.is_active?'🔒 User deactivated':'✅ User activated',{icon:u.is_active?'🔇':'🔔'});fetchAll()}};
  const delUser=async(id:number)=>{if(!confirm('⚠️ Delete this portal user permanently?'))return;const{error}=await supabase.from('school_portal_users').delete().eq('id',id);if(!error){toast.success('🗑️ Portal user deleted!',{icon:'✅'});fetchAll()}};
          ...
          <div className="relative px-7 py-6 overflow-hidden" style={{background:G.green}}>
            ...
            <div className="flex items-center justify-between relative z-10">
              <div className="flex items-center gap-3">
                ...
                <div><h2 className="text-xl font-extrabold text-white">{editItem?'Edit':'Add New'} Portal User</h2><p className="text-xs text-white/70 mt-0.5">{editItem?'Update portal access credentials':'Create login for parent or student'}</p></div>
              </div>
