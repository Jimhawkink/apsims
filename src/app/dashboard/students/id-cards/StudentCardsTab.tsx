...
import { FiCreditCard, FiPrinter, FiRefreshCw, FiFilter, FiDownload } from 'react-icons/fi';
            ...
            <div className="no-print bg-white rounded-2xl border border-gray-200 overflow-hidden">
                <div className="p-4">
                    <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
                        <div><label className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider mb-1 block">Form</label><select value={selForm} onChange={e => { setSelForm(e.target.value); setSelStream(''); }} className="select-modern w-full text-sm"><option value="">All Forms</option>{data.forms.map((f: any) => <option key={f.id} value={f.id}>{f.form_name}</option>)}</select></div>
                        <div><label className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider mb-1 block">Stream</label><select value={selStream} onChange={e => setSelStream(e.target.value)} className="select-modern w-full text-sm"><option value="">All Streams</option>{data.streams.map((s: any) => <option key={s.id} value={s.id}>{s.stream_name}</option>)}</select></div>
                        <div><label className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider mb-1 block">Search</label><div className="relative"><FiFilter size={11} className="absolute left-2.5 top-3 text-gray-400" /><input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Name or Adm No..." className="w-full pl-7 pr-3 py-2.5 border border-gray-200 rounded-lg text-[11px] focus:ring-2 focus:ring-indigo-200 focus:border-indigo-400 outline-none" /></div></div>
                        <div><label className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider mb-1 block">Template</label><select value={selTemplate?.id || ''} onChange={e => setSelTemplate(data.templates.find((t: any) => t.id === Number(e.target.value)) || null)} className="select-modern w-full text-sm"><option value="">Default</option>{data.templates.filter((t: any) => t.card_type === 'Student').map((t: any) => <option key={t.id} value={t.id}>{t.template_name}</option>)}</select></div>
                        <div className="flex items-end">
                            <label className="flex items-center gap-2 cursor-pointer py-2.5 group">
                                <input type="checkbox" checked={selectAll} onChange={e => setSelectAll(e.target.checked)} className="w-3.5 h-3.5 rounded accent-indigo-600" />
                                <span className="text-[11px] font-semibold text-gray-500 group-hover:text-gray-700 transition-colors">All ({filtered.length})</span>
                            </label>
                        </div>
                    </div>
                    <div className="flex flex-wrap gap-2 mt-3">
                        <button onClick={issueCards} className="px-3 py-2 text-[11px] font-bold text-white rounded-xl flex items-center gap-1.5 shadow-md transition-all hover:shadow-lg hover:scale-[1.02] active:scale-[0.98]" style={{ background: 'linear-gradient(135deg, #4f46e5, #7c3aed)', boxShadow: selStudents.size > 0 ? '0 4px 14px rgba(79,70,229,0.4)' : 'none' }}>
                            <FiCreditCard size={12} /> Issue Cards ({selStudents.size})
                        </button>
                        <button onClick={() => window.print()} disabled={selStudents.size === 0} className="px-3 py-2 text-[11px] font-bold text-white rounded-xl flex items-center gap-1.5 shadow-md disabled:opacity-40 transition-all hover:shadow-lg hover:scale-[1.02]" style={{ background: 'linear-gradient(135deg, #059669, #10b981)' }}>
                            <FiPrinter size={12} /> Print ({selStudents.size})
                        </button>
                        <button onClick={() => setShowBack(!showBack)} className="px-3 py-2 text-[11px] font-bold rounded-xl border border-indigo-200 text-indigo-600 flex items-center gap-1.5 hover:bg-indigo-50 transition-colors">
                            <FiRefreshCw size={12} /> {showBack ? 'Show Front' : 'Show Back'}
                        </button>
                    </div>
                </div>
                <div className="border-t border-gray-100 px-4 py-2 bg-gray-50/50 flex items-center gap-4">
                    <span className="text-[11px] font-bold text-gray-600">{filtered.length} Students</span>
                    <div className="w-px h-4 bg-gray-200" />
                    <span className="flex items-center gap-1 text-[11px]"><span className="w-2 h-2 rounded-full bg-green-500" /><strong className="text-green-700">{filtered.filter((s:any) => s.card_status === 'Issued').length}</strong> Issued</span>
                    <span className="flex items-center gap-1 text-[11px]"><span className="w-2 h-2 rounded-full bg-gray-400" /><strong className="text-gray-600">{filtered.filter((s:any) => !s.card_status || s.card_status !== 'Issued').length}</strong> Not Issued</span>
                    <span className="text-[11px] font-bold text-indigo-600 ml-auto">{selStudents.size} selected</span>
                </div>
            </div>
            ...
            <div className="no-print bg-white rounded-2xl border border-gray-200 overflow-hidden">
                <div className="max-h-[250px] overflow-y-auto">
                    <table className="w-full"><thead><tr className="bg-gray-50/80 border-b border-gray-200 sticky top-0">
                        ...
                        <th className="px-2 py-2 text-left text-[10px] font-bold text-gray-400 uppercase tracking-wider">Student</th>
                        <th className="px-2 py-2 text-left text-[10px] font-bold text-gray-400 uppercase tracking-wider">Form</th>
                        <th className="px-2 py-2 text-center text-[10px] font-bold text-gray-400 uppercase tracking-wider">Card</th>
                    </tr></thead><tbody>
                        {filtered.map((s: any) => (
                            <tr key={s.id} className={`border-b border-gray-100 hover:bg-indigo-50/40 cursor-pointer transition-all duration-150 ${selStudents.has(s.id) ? 'bg-indigo-50/60' : ''}`} onClick={() => toggle(s.id)}>
                                <td className="px-2.5 py-1.5"><input type="checkbox" checked={selStudents.has(s.id)} readOnly className="w-3.5 h-3.5 rounded pointer-events-none accent-indigo-600" /></td>
                                <td className="px-2 py-1.5 text-[11px] font-bold text-blue-600">{s.admission_no || s.admission_number}</td>
                                <td className="px-2 py-1.5 text-[11px] font-semibold text-gray-800">{s.first_name} {s.last_name}</td>
                                <td className="px-2 py-1.5 text-[11px] text-gray-600">{data.getFormName(s.form_id)}</td>
