...
export function VisitModal({ show, saving, onClose, onSave, form, setForm, students }: any) {
  ...
  return (
    <Overlay onClose={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-[720px] max-h-[94vh] overflow-hidden flex flex-col" style={{ animation: 'slideUp 0.3s ease-out' }}>
        <Header grad={G.teal} icon="🏥" title="Clinic Visit" subtitle="Record sick bay visit & notify parent" onClose={onClose} />
        <div className="p-6 space-y-5 overflow-y-auto flex-1">
          {/* Student Picker */}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5 mb-2">
              {commonComplaints.map(c => (
                <button key={c} type="button" onClick={() => s({ ...form, complaint: form.complaint ? `${form.complaint}, ${c}` : c })}
                  className="px-2.5 py-1 rounded-lg text-[10px] font-bold bg-teal-50 text-teal-700 border border-teal-200 hover:bg-teal-100 transition-all hover:scale-105 active:scale-95">{c}</button>
              ))}
            </div>
            <Textarea value={form.complaint} onChange={(v: string) => s({ ...form, complaint: v })} rows={2} placeholder="Describe complaint…" />
          </div>
          ...
          <div><Label>Medication Given</Label><Input value={form.medication_given} onChange={(v: string) => s({ ...form, medication_given: v })} placeholder="e.g. Paracetamol 500mg" /></div>

          {/* Vitals Strip */}
          <Section icon={<FiThermometer size={12} />} title="Vitals">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              <div><Label>Temp (°C)</Label>
                <input type="number" step="0.1" value={form.temperature} onChange={(e: any) => s({ ...form, temperature: e.target.value })} placeholder="37.0"
                  className={`w-full px-4 py-3 border rounded-xl text-sm font-bold focus:outline-none focus:ring-2 focus:ring-teal-100 transition-all ${tempClass || 'bg-gray-50 border-gray-200'}`} />
                {form.temperature && Number(form.temperature) >= 38 && <p className="text-[10px] text-red-500 font-bold mt-1 flex items-center gap-1">🔥 High Fever!</p>}
              </div>
          ...
          {/* Referral */}
          <div><Label>Referred To</Label><Input value={form.referred_to} onChange={(v: string) => s({ ...form, referred_to: v })} placeholder="e.g. Kenyatta National Hospital" /></div>
          <div><Label>Notes</Label><Textarea value={form.notes} onChange={(v: string) => s({ ...form, notes: v })} placeholder="Additional clinical notes…" /></div>

          {/* SMS Toggle */}
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-4 border border-blue-200">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-blue-100 flex items-center justify-center"><FiMessageSquare size={18} className="text-blue-600" /></div>
                <div>
                  <p className="text-sm font-bold text-blue-800">Notify Parent via SMS</p>
                  <p className="text-[10px] text-blue-500">Auto-send clinic visit alert to parent/guardian</p>
                </div>
              </div>
