...
export default function RemedialPage() {
  ...
  const getPaidForEnrollment = (studentId: number, termId: number) =>
    payments.filter(p => p.student_id === studentId && p.remedial_term_id === termId).reduce((s, p) => s + Number(p.amount), 0);
  ...
  const rosterData = useMemo(() => {
    ...
    const rows = enrollments
      .filter(e => e.remedial_term_id === termNum)
      .map(e => {
  ...
  const submitQuickPay = async () => {
    ...
    const { error } = await supabase.from('school_remedial_payments').insert([{
      ...
      remedial_term_id: Number(rosterTerm),
      amount: amt,
    ...
    if (credit > 0) {
      ...
      if (nextTerm) {
        const alreadyEnrolled = enrollments.find(e => e.student_id === quickPayTarget.studentId && e.remedial_term_id === nextTerm.id);
        if (!alreadyEnrolled) {
  ...
  const submitDelete = async () => {
    ...
    await supabase.from('school_remedial_payments').delete().eq('student_id', deleteTarget.studentId).eq('remedial_term_id', Number(rosterTerm));
  ...
  const handleMassEnroll = async () => {
    ...
    const alreadyEnrolled = new Set(enrollments.filter(e => e.remedial_term_id === termNum).map(e => e.student_id));
        ...
        {stmtTarget && (() => {
          ...
          return (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(150px,1fr))', gap: 10 }}>
                {se.map(e => {
                  const paid = getPaidForEnrollment(stmtTarget.studentId, e.remedial_term_id);
