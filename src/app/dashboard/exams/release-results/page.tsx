...
import { FiSend, FiLock, FiUnlock, FiRefreshCw, FiSmartphone, FiFilter, FiChevronDown, FiZap } from 'react-icons/fi';
...
export default function ReleaseResultsPage() {
    ...
    const load = useCallback(async () => {
        ...
        const [tR, fR, stR, relR, lockR, muR] = await Promise.all([
            ...
            supabase.from('school_mobile_users').select('*, school_students(first_name,last_name,admission_number,form_id,school_forms(form_name,form_level))').eq('is_active', true),
        ]);
        setTerms(tR.data || []);
        setForms(fR.data || []);
        setStreams(stR.data || []);
    ...
    useEffect(() => {
        ...
        supabase.from('school_exam_marks')
            ...
            .then(({ data }) => {
                ...
                (data || []).forEach((r:any) => { const k = String((r.school_students as any)?.form_id); counts[k] = (counts[k]||0)+1; });
    ...
    const globalRelease = releases.find(r => String(r.term_id) === selTerm && !r.form_id && r.exam_type === selExam && r.is_released);
    const totalMobile = mobileUsers.filter(mu => !selForm || (mu.school_students as any)?.form_id === Number(selForm)).length;
    ...
    const releaseForm = async (formId: number, formName: string, streamId?: number, streamName?: string) => {
        ...
        if (targets.length) await supabase.from('school_mobile_notifications').insert(targets.map(mu => ({
            student_id: mu.student_id, guardian_phone: mu.guardian_phone,
            title: 📊  Results Out,
            body: releaseMsg, type: 'results', data: { term_id: Number(selTerm), exam_type: selExam },
        })));
        setSaving(null); toast.success(✅  released!); load();
    };

    const revokeRelease = async (formId: number | null, streamId?: number) => {
        if (!confirm('Revoke? Parents will see a locked screen immediately.')) return;
        setSaving('revoke');
        let q = supabase.from('school_report_releases').update({ is_released: false })
            .eq('term_id', Number(selTerm)).eq('exam_type', selExam);
        if (formId === null) q = q.is('form_id', null); else q = q.eq('form_id', formId);
        if (streamId) q = (q as any).eq('stream_id', streamId);
        await q; setSaving(null); toast.success('🔒 Revoked'); load();
    };

    const toggleLock = async (formId: number, formName: string, locked: boolean) => {
        const user = getUser(); const uname = user.full_name || 'Admin';
        await supabase.from('school_marks_lock').upsert({
            term_id: Number(selTerm), form_id: formId, exam_type: selExam, is_locked: !locked,
            locked_by: !locked ? uname : undefined, locked_at: !locked ? new Date().toISOString() : undefined,
            unlocked_by: locked ? uname : undefined, unlocked_at: locked ? new Date().toISOString() : undefined,
        }, { onConflict: 'term_id,form_id,exam_type', ignoreDuplicates: false });
        toast.success(!locked ? 🔒  locked : 🔓  unlocked); load();
    };
    ...
    return (
        <div className="space-y-6 pb-16">
            ...
            <div className="rounded-2xl p-6 text-white" style={{ background:'linear-gradient(135deg,#0c4a6e,#0891b2,#7c3aed)' }}>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-5">
                    {[
                        ...
                        { l:'Released Forms',  v: formStatus.filter(s => s.release).length, i:'✅' },
                        { l:'Total Marks',     v: Object.values(markCounts).reduce((a,b)=>a+b,0), i:'📊' },
                        { l:'Locked Forms',    v: formStatus.filter(s => s.lock?.is_locked !== false).length, i:'🔒' },
                    ].map(k => (
            ...
            {/* ══ GLOBAL RELEASE ════════════════════════════ */}
            <div className={ounded-2xl border-2 p-5 + (globalRelease ? 'border-green-300 bg-green-50' : 'border-amber-200 bg-amber-50')}>
                <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                    <div>
                        <p className="font-black text-gray-800 text-lg">{globalRelease ? '✅' : '🔒'} Global Release — All Forms</p>
                        <p className="text-sm text-gray-500 mt-0.5">
                            {globalRelease ? Released by  ·  : Not released for  }
                        </p>
                        {globalRelease && <p className="text-xs text-green-600 font-bold mt-1">📱 {totalMobile} parent(s) can now view results</p>}
                    </div>
                    <div className="flex gap-2">
                        {globalRelease
                            ? <button onClick={() => revokeRelease(null)} disabled={saving==='revoke'} className="px-4 py-2 text-xs font-black text-red-700 bg-red-50 hover:bg-red-100 border border-red-200 rounded-xl transition flex items-center gap-1">🔒 Revoke All</button>
                            : <button onClick={releaseAll} disabled={saving==='all'} className="px-5 py-2.5 text-sm font-black text-white bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 rounded-xl transition flex items-center gap-2 shadow-md disabled:opacity-50">
                                🚀 {saving==='all' ? 'Releasing…' : 'Release ALL Forms to Mobile'}
                              </button>
                        }
                    </div>
                </div>
            ...
            <div>
                <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
                    <p className="font-black text-gray-800">
                        ...
                        {filteredForms.length < forms.length && <span className="ml-2 text-xs text-indigo-600 font-bold bg-indigo-50 px-2 py-0.5 rounded-full">{filteredForms.length}/{forms.length} forms</span>}
                    </p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                    {formStatus.map(({ form, release, lock, marks, parents }) => {
                        ...
                        const key        = ;
                        const saving_key = saving === key;

                        return (
                            <div key={form.id} className={g-white rounded-2xl border-2 shadow-sm p-5 space-y-3 transition + (isReleased ? 'border-green-200' : 'border-gray-100')}>
                                {/* Header */}
                                <div className="flex items-center justify-between">
                                    <div>
                                        <p className="font-black text-gray-800 text-base">{isCBC ? '🌱' : '🎓'} {form.form_name}</p>
                                        <p className="text-[10px] text-gray-400 font-bold uppercase mt-0.5">{isCBC ? 'CBC' : '8-4-4'} · Level {form.form_level} · {selExam}</p>
                                    </div>
                                    <div className="flex flex-col gap-1 items-end">
                                        <span className={	ext-[10px] font-black px-2 py-0.5 rounded-full +(isLocked ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700')}>{isLocked ? '🔒 Locked' : '🔓 Open'}</span>
                                        <span className={	ext-[10px] font-black px-2 py-0.5 rounded-full +(isReleased ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-500')}>{isReleased ? '📱 Released' : '⏸ Unreleased'}</span>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-3 gap-2 bg-gray-50 rounded-xl p-3">
                                    {[{ l:'Marks', v: marks }, { l:'Parents', v: parents }, { l:'Level', v: form.form_level }].map(s => (
                                        <div key={s.l} className="text-center"><p className="text-lg font-black text-gray-800">{s.v}</p><p className="text-[9px] text-gray-400 font-bold uppercase">{s.l}</p></div>
                                    ))}
                                </div>

                                {release?.released_by && <p className="text-xs text-gray-400">Released by <strong>{release.released_by}</strong> · {fmt(release.released_at)}</p>}
                                {lock?.locked_by && <p className="text-xs text-gray-400">Locked by <strong>{lock.locked_by}</strong> · {fmt(lock.locked_at)}</p>}
                                ...
                                <div className="flex gap-2 flex-wrap">
                                    ...
                                    {isReleased
                                        ? <button onClick={() => revokeRelease(form.id)} className="px-3 py-2 text-xs font-black text-red-700 bg-red-50 hover:bg-red-100 rounded-xl flex items-center gap-1">🔒 Revoke</button>
                                        : <button onClick={() => releaseForm(form.id, form.form_name)} disabled={saving_key}
                                            ...
                                            📤 {saving_key ? 'Releasing…' : 'Release Form'}
                                          </button>
                                    }
                                </div>
                                ...
                                <details className="group">
                                    <summary className="cursor-pointer text-[11px] font-bold text-indigo-600 hover:text-indigo-800 list-none flex items-center gap-1 mt-1">
                                        ▶ Release by Stream (Individual)
                                    </summary>
                                    <div className="mt-2 space-y-1.5 pl-2">
                                        {streams.map(stream => {
                                            const sRel = releases.find(r => r.form_id === form.id && r.stream_id === stream.id && String(r.term_id) === selTerm && r.exam_type === selExam && r.is_released);
                                            const sk   = _s;
                                            return (
                                                <div key={stream.id} className="flex items-center justify-between bg-gray-50 rounded-lg px-3 py-1.5">
                                                    <span className="text-xs font-semibold text-gray-700">📌 {form.form_name} {stream.stream_name}</span>
                                                    <div className="flex gap-1.5 items-center">
                                                        {sRel && <span className="text-[9px] font-black text-green-600 bg-green-100 px-1.5 py-0.5 rounded-full">Released</span>}
                                                        {sRel
                                                            ? <button onClick={() => revokeRelease(form.id, stream.id)} className="text-[10px] font-bold text-red-600 hover:text-red-800 underline">Revoke</button>
                                                            : <button onClick={() => releaseForm(form.id, form.form_name, stream.id, stream.stream_name)} disabled={saving === sk}
                                                                className="text-[10px] font-black text-blue-600 hover:text-blue-800 bg-blue-50 hover:bg-blue-100 px-2 py-1 rounded-lg">
                                                                {saving === sk ? '…' : '📤 Release'}
                                                              </button>
                                                        }
                                                    </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                <div className="p-4 border-b border-gray-100">
                    <p className="font-black text-gray-800">👨‍👩‍👧 Registered Mobile Parents ({mobileUsers.filter(mu => !selForm || (mu.school_students as any)?.form_id === Number(selForm)).length})</p>
                ...
                {mobileUsers.length === 0 ? (
                    <div className="p-12 text-center">
                        ...
                        <p className="text-xs text-gray-300 mt-1">Parents self-register using their guardian phone number in the APSIMS app</p>
                    </div>
                ) : (
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {mobileUsers.filter(mu => !selForm || (mu.school_students as any)?.form_id === Number(selForm))
                                    .map((mu, i) => {
                                        ...
                                        return (
                                            <tr key={mu.id} className="hover:bg-gray-50">
                                                ...
                                                <td className="px-4 py-2.5 text-xs font-semibold whitespace-nowrap">{st ? ${st.first_name}  : #}</td>
                                                ...
                                                <td className="px-4 py-2.5"><span className={	ext-[10px] font-black px-2 py-0.5 rounded-full +(mu.is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600')}>{mu.is_active ? '✅ Active' : '❌ Inactive'}</span></td>
                                                <td className="px-4 py-2.5 text-xs text-gray-400 whitespace-nowrap">{mu.last_login ? fmt(mu.last_login) : 'Never'}</td>
                                            </tr>
                                        );
                                    })}
                            </tbody>
